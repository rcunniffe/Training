...
namespace training_rc
{
    public partial class PlaceOrders : System.Web.UI.Page
    {
        ...
        public void sunmitbtn_Click(object sender, EventArgs e)
        {
            ...
            isTextValid = TextValidate(isTextValid);
            if (isTextValid)
            {
                try
                {
            ...
            else { Response.Write("<script type='text/javascript'>alert('The values did not comform');</script>"); }          
        }

        /// <summary>
        /// Validates the Form on placeOrders.aspx
        /// </summary>
        /// <param name="isTextValid">if set to <c>true</c> [is text valid].</param>
        /// <returns></returns>
        protected bool TextValidate(Boolean isTextValid)
        {
            isTextValid = false;                   
